...
public class ThemeResource {
     ...
     * or with status 500 (Internal Server Error) if the themeDTO couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @RequestMapping(value = "/themes",
    ...
    public ResponseEntity<ThemeDTO> updateTheme(@RequestBody ThemeDTO themeDTO) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert("theme", themeDTO.getId().toString()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if there is an error to generate the pagination HTTP headers
     */
    @RequestMapping(value = "/_search/themes",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<ThemeDTO>> searchThemes(@RequestParam String query, Pageable pageable)
        throws URISyntaxException {
        log.debug("REST request to search for a page of Themes for query {}", query);
        Page<ThemeDTO> page = themeService.search(query, pageable);
